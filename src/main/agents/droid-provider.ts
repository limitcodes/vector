...
const DROID_DEFAULT_MODEL_ID = 'droid-default'

const DROID_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="85 85 338 338" fill="none">
  <path
...
const DROID_THINKING_LEVELS: ThinkingLevel[] = ['off', 'minimal', 'low', 'medium', 'high', 'xhigh']

const DROID_REASONING_BY_THINKING_LEVEL: Record<ThinkingLevel, ReasoningEffort> = {
...
const THINKING_LEVEL_BY_DROID_REASONING = new Map<ReasoningEffort, ThinkingLevel>([
  [ReasoningEffort.Off, 'off'],
  [ReasoningEffort.None, 'off'],
  [ReasoningEffort.Minimal, 'minimal'],
  [ReasoningEffort.Low, 'low'],
  [ReasoningEffort.Medium, 'medium'],
  [ReasoningEffort.Dynamic, 'medium'],
...
const toDroidImages = (
  ...
  return images
    .map((image) => {
      const parsed = Base64ImageSourceSchema.safeParse({
        type: 'base64' as const,
        data: image.data,
        mediaType: image.mimeType.toLowerCase()
      })
...
const getSessionKey = (chatId: string, cwd: string, modelId: string): string =>
  `${cwd}:${chatId}:${modelId}`

const getFallbackModel = (): AgentModel => ({
  id: DROID_DEFAULT_MODEL_ID,
  name: 'Droid default',
  ...
  optionGroups: [
    {
      id: 'thinking',
      label: 'Thinking',
      options: DROID_THINKING_LEVELS.map((level) => ({
        id: level,
        label: level
      }))
    }
...
const getThinkingOptionsForModel = (
  model: AvailableModelConfig
): Array<{ id: ThinkingLevel; label: string }> => {
  const levels = model.supportedReasoningEfforts
    .map((effort) => THINKING_LEVEL_BY_DROID_REASONING.get(effort))
    .filter((level): level is ThinkingLevel => Boolean(level))
  const uniqueLevels = [...new Set(levels)]
  const orderedLevels = DROID_THINKING_LEVELS.filter((level) => uniqueLevels.includes(level))

  return (orderedLevels.length > 0 ? orderedLevels : DROID_THINKING_LEVELS).map((level) => ({
    id: level,
    label: level
  }))
}

export const createDroidProvider = (): AgentProvider => {
  const sessionCache = new Map<string, DroidSession>()
  let modelCache: AgentModel[] | undefined

  const getModels = async (): Promise<AgentModel[]> => {
      ...
      modelCache =
        availableModels.length > 0
          ? availableModels.map((model) => ({
              ...
              name: model.displayName || model.shortDisplayName || model.modelId || model.id,
              providerId: DROID_PROVIDER_ID,
              optionGroups: [
                {
                  id: 'thinking' as const,
                  label: 'Thinking',
                  options: getThinkingOptionsForModel(model)
                }
              ]
            }))
          : [getFallbackModel()]
  ...
  const getOrCreateSession = async (
    ...
    const key = getSessionKey(chatId, cwd, modelId)
    const cached = sessionCache.get(key)
    const reasoningEffort = DROID_REASONING_BY_THINKING_LEVEL[thinkingLevel]

    if (cached) {
      await cached.updateSettings({ reasoningEffort })
      return cached
    }

    const session = await createSession({
      cwd,
      ...(modelId !== DROID_DEFAULT_MODEL_ID ? { modelId } : {}),
      reasoningEffort,
      enabledToolIds: ['Read', 'Edit', 'Write', 'Execute']
    })
  ...
  return {
    metadata: {
      id: DROID_PROVIDER_ID,
      name: 'Droid',
      iconSvg: DROID_ICON_SVG
    },
    isConfigured: () => true,
    getModels,
    sendMessage: async ({
      ...
      try {
        ...
        for await (const message of session.stream(prompt, {
          images: toDroidImages(images)
        })) {
          if (message.type === DroidMessageType.AssistantTextDelta) {
            emit({ type: 'text_delta', chatId, requestId, delta: message.text })
            continue
          }

          if (message.type === DroidMessageType.ThinkingTextDelta) {
            emit({ type: 'thinking_delta', chatId, requestId, delta: message.text })
            continue
          }
          ...
          if (message.type === DroidMessageType.ToolUse) {
            emit({
              ...
              argsText: safeStringify(message.toolInput)
            })
            continue
          }

          if (message.type === DroidMessageType.ToolProgress) {
            emit({
              type: 'tool_update',
              chatId,
              requestId,
          ...
          if (message.type === DroidMessageType.ToolResult) {
            emit({
              ...
              toolCallId: message.toolUseId,
              toolName: message.toolName,
              output: safeStringify(message.content),
          ...
          if (message.type === DroidMessageType.Error) {
            emit({ type: 'error', chatId, requestId, error: message.message })
            return
          }
        }
